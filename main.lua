--[[
    SpectrumX Fluent Single-File UI Library
    - Single file
    - No require(ModuleScript)
    - Executor-friendly
    - Floating button included
    - Responsive behavior included
    - Fluent-inspired structure, SpectrumX-style usability
]]
...
local function IsImageIcon(icon)
    return type(icon) == "string" and (
        string.find(icon, "rbxassetid://") ~= nil
...
function WindowClass:_createFloatingButton()
    ...
    local position = string.lower(cfg.Position or "BottomRight")
    ...
    elseif position == "topleft" then
        ...
        holder.Position = UDim2.new(0, 18, 0, 18)
    else
    ...
    local icon = CreateIcon(buttonFrame, cfg.Icon or "settings", self.Theme.Text, 18, 0)
    if icon then
    ...
    if cfg.Draggable then
        MakeDraggable(buttonFrame, holder)
    end
